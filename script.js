document.addEventListener('DOMContentLoaded', () => {
    ...
    window.app = {
        ...
        // KNOWLEDGE BASE: Ethics in an Entropic World (2026)
        frameworkContext: `
            You are the 'Ethical Cartography' AI Tutor. 
            CORE PRINCIPLES:
            1. Phenomenal Reference Frame (PRF): Subjective horizon of meaning.
            2. Multi-Scale Coherence (BROA+): Biological (BRC), Identity (KIC), Cognitive (CIC), Institutional (ISC), and Temporal (TNC).
            3. ATCF: Metric for narrative and prospective integrity.
            4. Sapolsky Function: High entropy causing temporal collapse.
            TASK: Diagnose coherence failures and map functional equivalents.
        `,

        init() {
            // Priority 1: Setup interaction so buttons don't "lock up"
            this.setupEventListeners();
            
            // Priority 2: Render data
            try {
            ...
            // Priority 3: Pre-fill key if exists
            if (this.apiKey) {
        ...
        async callGeminiAPI(prompt, outputElement) {
            if (!this.apiKey) {
                outputElement.innerHTML = `<div class="p-6 bg-red-50 text-red-700 rounded-2xl font-black text-xs uppercase">⚠️ Missing API Key. Go back and enter it.</div>`;
                return;
            }

            outputElement.innerHTML = `<div class="flex items-center gap-4 p-8 bg-indigo-50 rounded-3xl animate-pulse">
                <span class="text-indigo-800 font-black text-sm uppercase tracking-widest italic">Analyzing Multi-Scale Coherence...</span>
            </div>`;

            try {
                // Endpoint version check for 2026
                ...
                if (text) {
                    ...
                    outputElement.innerHTML = `<div class="bg-white border-2 border-indigo-100 p-8 rounded-[2rem] text-slate-700 text-sm leading-relaxed">${formatted}</div>`;
                }
            } catch (e) {
        ...
        setupEventListeners() {
            ...
            if (startBtn) {
                startBtn.onclick = () => {
                    ...
                    if (!this.apiKey) {
                        alert("Please enter your API Key to initialize the lab.");
                        return;
                    }
            ...
            return `
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div class="bg-indigo-900 p-10 rounded-[3rem] text-white shadow-2xl relative">
                        ...
                        <div id="modal-chat-output" class="text-sm mb-8 space-y-4 max-h-[300px] overflow-y-auto pr-4"></div>
