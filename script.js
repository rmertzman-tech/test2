document.addEventListener('DOMContentLoaded', () => {
    ...
    window.app = {
        ...
        frameworkContext: `
            ...
            - PRF stands for Phenomenal Reference Frame (The agent's lived horizon of meaning).
            - Multi-Scale Coherence (BROA+): Agency requires alignment across Biological (BRC), Identity (KIC), Cognitive (CIC), Institutional (ISC), and Temporal (TNC) constraints.
            - ATCF: Measures narrative and prospective integrity (At, Tt, Ct, Ft).
            - Landauer Principle: Information erasure and ethical overwriting have physical energetic costs.
            - Sapolsky Function: Chronic stress triggers temporal collapse, shrinking the horizon of agency.
        `,

        init() {
            this.setupEventListeners();
            try { this.renderAllContent(); } catch (e) { console.error("Render Error:", e); }
            if (this.apiKey) {
        ...
        createCardHtml(item, type, index) {
            ...
            return `
                <article role="button" data-type="${type}" data-index="${index}" 
                         class="profile-card bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 hover:shadow-2xl transition-all cursor-pointer flex flex-col">
        ...
        createSimpleCardHtml(item, type, index) {
            return `
                <article role="button" data-type="${type}" data-index="${index}" class="simple-card bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 hover:shadow-xl transition-all cursor-pointer flex flex-col">
    ...
    try {
        ...
        getPersonHtml(data) {
            return `
                <div class="mb-12 text-center">
                    ...
                    <p class="text-indigo-600 font-black uppercase tracking-widest text-[10px]">${data.title}</p>
                </div>
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div class="space-y-8 pr-4">
                        ...
                        <section><h4 class="text-[10px] font-black uppercase text-slate-400 mb-2 tracking-widest">Temporal Coherence (ATCF)</h4><p class="text-slate-500 text-sm italic">${data.atcf}</p></section>
                    </div>
                    <div class="bg-indigo-900 p-10 rounded-[3rem] text-white shadow-2xl relative">
                        ...
                        <div class="flex gap-2 p-3 bg-white/10 rounded-2xl border border-white/20">
                            <input type="text" id="modal-chat-input" class="flex-1 bg-transparent border-none focus:ring-0 text-white placeholder-indigo-300 text-sm" placeholder="Ask through PRF...">
                            <button id="modal-chat-send" class="bg-white text-indigo-900 px-6 py-3 rounded-xl font-black hover:bg-indigo-100">SEND</button>
                        </div>
                    </div>
                </div>`;
        },
        ...
        async handleResonanceLab() {
            ...
            if (!input) return;
            const prompt = `${this.frameworkContext}\nStudent PRF Reflection: "${input}"\nIdentify role models and explain Functional Equivalence using multi-scale coherence metrics.`;
